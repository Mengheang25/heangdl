...
function renderMediaSlides(container, items, resultThumbnail) {
  ...
  container.querySelectorAll(".heang-player-container").forEach((pc) => {
...
const isDataSaver = localStorage.getItem("heangdl_data_saver") === "true";
  ...
  items.forEach((dl, index) => {
    ...
    if (isVideo) {
      ...
      const autoPlaySetting = localStorage.getItem("heangdl_autoplay") !== "false";
  ...
  img.onerror = () => {
    ...
    if (!img.dataset.retry) {
      ...
      if (window.Capacitor?.isNativePlatform() && CapacitorHttp) {
        ...
        // Web App mode fallback: Route through server proxy endpoint
        img.src = `/api/proxy/media?url=${encodeURIComponent(targetUrl)}&referer=${encodeURIComponent(referer)}`;
      }
    } else {
      img.style.display = "none";
    }
  ...
  slides.forEach((slide, index) => {
    ...
    if (index === currentSlideIndex) {
      ...
      if (video) {
        ...
        video.currentTime = 0;
        video.loop = localStorage.getItem("heangdl_loop") !== "false";
        video.play().catch(() => {});
      }
    } else {
...
export async function startNativeDownload(url, type, title, btn, sourceUrl) {
  ...
  if (window._heangdlActiveSimInterval) {
    clearInterval(window._heangdlActiveSimInterval);
    window._heangdlActiveSimInterval = null;
  }
  ...
  try {
    ...
    window._heangdlActiveSimInterval = setInterval(() => {
    ...
    if (window._heangdlProgressListener) {
      try {
        await window._heangdlProgressListener.remove();
      } catch (_) {}
      window._heangdlProgressListener = null;
    }

    // Listen for real progress if Filesystem exists
    if (Filesystem?.addListener) {
      try {
        window._heangdlProgressListener = await Filesystem.addListener(
          "downloadProgress",
          (progress) => {
    ...
    let sanitizedTitle = (title || "HEANG D.L Media")
      .replace(/[\\/:*?"<>|#%&{}[\]()@$^+=~`';,]/g, "")
    ...
    if (!sanitizedTitle) sanitizedTitle = "HEANG D.L_Media";

    const template = localStorage.getItem("heangdl_filename") || "title";
    ...
    const videoSubfolder = localStorage.getItem("heangdl_download_path") || "HEANG D.L";
    const musicSubfolder =
      localStorage.getItem("heangdl_music_path") || "HEANG D.L/Music";
    ...
    if (localStorage.getItem("heangdl_auto_folder") === "true") {
      const src = (sourceUrl || url || "").toLowerCase();
    ...
    let savedFile;
    let attempts = 0;
    const isAutoRetry = localStorage.getItem("heangdl_auto_retry") !== "false";
    ...
    if (localStorage.getItem("heangdl_auto_clear_input") === "true") {
      const urlInput = document.getElementById("urlInput");
    ...
    window.dispatchEvent(
      new CustomEvent("heangdl_file_saved", {
  ...
  try {
    ...
    if (window.Capacitor?.isNativePlatform()) {
      ...
      reader.onloadend = async () => {
        ...
        try {
          await Filesystem.writeFile({
            path: `Download/HEANG D.L/${fileName}`,
            data: base64,
            directory: "EXTERNAL_STORAGE",
            recursive: true,
          });
      ...
      if (tauriInvoke) {
        try {
          const customFolder =
            localStorage.getItem("heangdl_download_path") || "HEANG D.L";
  ...
  if (shareBtn) {
    shareBtn.addEventListener("click", () => {
      if (navigator.share) {
        ...
        const shareUrl = "https://heangdl.netlify.app/";
        if (window.Capacitor?.isNativePlatform()) {
          // Native share implementation for Capacitor
          window.Capacitor.Plugins.Share.share({
            title: "HEANG D.L App",
            text: "Check out HEANG D.L App",
            url: shareUrl,
            dialogTitle: "Share HEANG D.L App",
          });
        } else {
          // Fallback for web
          window.open(
            `https://twitter.com/intent/tweet?text=Check out HEANG D.L App&url=${encodeURIComponent(shareUrl)}`,
            "_blank",
          );
        }
      }
