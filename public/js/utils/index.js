...
export function getUserAgent() {
  const mode = localStorage.getItem("heangdl_user_agent") || "default";
...
export async function triggerHaptic(type = "medium") {
  if (localStorage.getItem("heangdl_haptic") === "false") return;
  try {
...
export function handleScrapeError(err, status = null) {
  ...
  if (status === 403 || status === 429) {
    msg = "IP Blocked or Rate Limited! Please try another server or use VPN.";
  } else if (
    ...
    if (err.message.includes("Failed to fetch") || err.message.includes("NetworkError")) {
      ...
      if (isLocal) {
        msg = "Local Proxy Server offline. Please start server with 'npm start'.";
      } else {
        msg = "Cloud Network Connection Error. Please test another server.";
      }
    } else {
      msg = err.message;
    }
...
export async function requestWakeLock() {
  if (
    localStorage.getItem("heangdl_keep_awake") === "true" &&
