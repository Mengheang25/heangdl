...
async function scrapeSnapTik(cleanUrl, originalUrl) {
  ...
  const photos = info.photoUrls || info.photos || info.images || info.slides;
  ...
  if (!downloads.length) throw new Error("No download links found from SnapTik.");
...
async function scrapeTikTokIO(cleanUrl, originalUrl) {
...
export async function scrapeTikTok(url) {
  ...
  try {
    ...
    const preferred = _ttSource || "snaptik";
    _ttSource = null;

    try {
      if (preferred === "snaptik") {
        return await scrapeSnapTik(cleanUrl, url);
      } else {
        return await scrapeTikTokIO(cleanUrl, url);
      }
    } catch (primaryErr) {
      console.warn(`[TIKTOK PRIMARY FAILED (${preferred})]:`, primaryErr.message);
      const fallback = preferred === "snaptik" ? "tiktokio" : "snaptik";
      try {
        if (fallback === "snaptik") {
          return await scrapeSnapTik(cleanUrl, url);
        } else {
