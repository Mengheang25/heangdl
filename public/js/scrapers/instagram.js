...
async function scrapeDownReels(cleanUrl, url) {
...
async function scrapeInDown(cleanUrl, url) {
...
export async function scrapeInstagram(url) {
  ...
  try {
    ...
    const preferred = _igSource || "downreels";
    _igSource = null;

    try {
      if (preferred === "downreels") {
        return await scrapeDownReels(cleanUrl, url);
      } else {
        return await scrapeInDown(cleanUrl, url);
      }
    } catch (primaryErr) {
      console.warn(`[INSTAGRAM PRIMARY FAILED (${preferred})]:`, primaryErr.message);
      const fallback = preferred === "downreels" ? "indown" : "downreels";
      try {
        if (fallback === "downreels") {
          return await scrapeDownReels(cleanUrl, url);
        } else {
          return await scrapeInDown(cleanUrl, url);
        }
