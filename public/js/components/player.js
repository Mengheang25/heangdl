...
/**
 * Creates a custom video player element with all HeangDLPlayer controls.
...
export function createVideoPlayer(dl, index, resultThumbnail) {
  ...
  playerContainer.className = "heang-player-container";
  ...
  const removeLoading = () => {
    playerContainer.classList.remove("heang-loading");
    removeFallbackImg();
  };
  ...
  if (isLocal && (isNative || tauriConvertFileSrc || tauriInvoke)) {
    playerContainer.classList.add("heang-loading");
        ...
        .then((res) => {
          if (
            ...
            const autoPlaySetting =
              localStorage.getItem("heangdl_autoplay") !== "false";
            if (
  ...
  video.onwaiting = () => playerContainer.classList.add("heang-loading");
  ...
  video.onerror = async (e) => {
    ...
    const ctrlEl = playerContainer.querySelector(".heang-player-controls");
    if (ctrlEl) ctrlEl.remove();

    if (!playerContainer.querySelector(".heang-player-error") && !playerContainer.querySelector(".fallback-img")) {
      ...
      if (fallbackSrc) {
        ...
        errOverlay.className = "heang-player-error";
  ...
  bigPlay.className = "heang-player-big-play visible";
  ...
  controls.className = "heang-player-controls";
  controls.innerHTML = `
    <div class="heang-player-progress">
      <div class="heang-player-progress-inner"></div>
    </div>
    <div class="heang-player-bottom">
      <div class="heang-player-actions">
        <button class="heang-player-btn play-toggle">
        ...
        <span class="heang-player-time">0:00 / 0:00</span>
      </div>
      <div class="heang-player-actions">
        <button class="heang-player-btn mute-toggle">
        ...
        <button class="heang-player-btn fullscreen-btn">
  ...
  const timeDisplay = controls.querySelector(".heang-player-time");
  const prog = controls.querySelector(".heang-player-progress");
  const progInner = controls.querySelector(".heang-player-progress-inner");
