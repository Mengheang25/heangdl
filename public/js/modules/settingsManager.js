...
export function exportHeangDLData() {
  try {
    const backup = {
      version: "4.2.0",
      timestamp: Date.now(),
    ...
    a.href = url;
    a.download = `HeangDL_Backup_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
...
export function importHeangDLData(file, onComplete) {
  if (!file) return;
