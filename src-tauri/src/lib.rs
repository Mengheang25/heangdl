...
async fn tauri_download_file(
    ...
    let mut target_dir = download_dir.join("HEANG D.L");

    if let Some(f) = folder {
        let trimmed = f.trim();
        if !trimmed.is_empty() {
            if trimmed.starts_with("HEANG D.L/") || trimmed.starts_with("HEANG D.L\\") {
                target_dir = download_dir.join(trimmed);
            } else if trimmed != "HEANG D.L" {
    ...
    if target_file.exists() {
        ...
        let stem = p.file_stem().and_then(|s| s.to_str()).unwrap_or("HEANG D.L_Media");
...
async fn tauri_read_file_bytes(path: String) -> Result<Vec<u8>, String> {
    ...
    } else if rel_path.starts_with("HEANG D.L/") {
        download_dir.join(rel_path)
    } else {
        download_dir.join("HEANG D.L").join(rel_path)
    };
...
async fn tauri_save_bytes_file(
    ...
    if target_file.exists() {
        let p = std::path::Path::new(&filename);
        let stem = p.file_stem().and_then(|s| s.to_str()).unwrap_or("HEANG D.L_Document");
